...
3. 如果你的 agent 支持仓库内命令，可以直接输入：

   ```text
   /init
   ```

   这会让 agent 优先扫描本地证据，并创建或更新私有 `EVIDENCE_SOURCES.md` 与 `EVIDENCE_SOURCES_CHANGELOG.md`。

4. 如果你的 agent 不支持仓库内 `/init` 命令，手动将 `EVIDENCE_SOURCES.example.md` 复制为 `EVIDENCE_SOURCES.md`，并将 `EVIDENCE_SOURCES_CHANGELOG.example.md` 复制为 `EVIDENCE_SOURCES_CHANGELOG.md`。
5. 也可以对 agent 说：
